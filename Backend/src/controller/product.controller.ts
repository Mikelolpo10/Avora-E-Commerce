...
import type { Product, ProductWithCategory } from '../interfaces/product.interface.js';
...
export const getSimilarProducts = async (req: Request, res: Response) => {
  ...
  try {
    const productResult = await pool.query(
      `SELECT category_id, department, material, price FROM products WHERE slug = $1`,
      [slug]
    );
    ...
    const result = await pool.query(
      ...
      SELECT *,
        (
          ...
          CASE WHEN price BETWEEN $4 * 0.8 AND $4 * 1.2 THEN 10 ELSE 0 END
        ) AS similarity_score
      FROM products
      WHERE slug != $5 AND status = 'active'
      ORDER BY similarity_score DESC
      LIMIT 6;
      `,
      [product.category_id, product.department, product.material, product.price, slug]
    );
